...
def get_net_features(Net, data_in, labels_in, img_ids):
    ...
    data_in = data_in.permute(0, 3, 1, 2)
    labels_in = torch.from_numpy(labels_in).type(torch.float32)
    dim_in = data_in.shape[-1]
    global outputs
    outputs = []
    Net.ResNet.fc.register_forward_hook(hook)
    allAccuracy =[]
    allWrongs = []
    predictions = []
    labels = []
    ...
    for i, img_id in enumerate(img_ids):
        if i >= len(predictions):
            continue
        result[img_id] = {}
        result[img_id]['prediction'] = predictions[i]
        result[img_id]['label'] = test_labels[i]
        result[img_id]['activations'] = activations[i]
    return result

def test_reg(batch_size, testData, test_labels, Net, dimIn, includePredictionLabels=False, test_eval=True):
    allAccuracy =[]
    allWrongs = []
    predictions = []
    labels = []
    if test_eval:
        Net.eval()
    for batch_idx, (data, target) in enumerate(batch_gen(testData, test_labels, 1, shuffle=False)):
